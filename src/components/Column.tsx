...
export function ColumnView(props: { notesDb: Notes; notesList: Note[] }) {
  ...
  //   const index = note.lines.indexOf(focusedBlockId);
  //   if (index >= note.lines.length - 1) {
  //     // At the bottom of a note, so move to the note below
  //     const blocks = getNoteBelow(note.id)?.lines;
  //     const firstBlock = blocks?.[0];
  //     if (firstBlock) {
  //       setFocus(firstBlock.id);
  //     }
  //   } else {
  //     setFocus(note.lines[index + 1]);
  //   }
  // };
  ...
  //     const blocks = getNoteAbove(note.id)?.lines;
